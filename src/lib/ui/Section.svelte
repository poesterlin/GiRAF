<script lang="ts">
	...
	let { title, section, children }: Props = $props();
</script>
...
<details class="border-b border-neutral-700/50">
	<summary class="mb-1 flex cursor-pointer items-center rounded-lg bg-neutral-800 px-4 py-2 select-none">
		...
		<Checkbox label={title} bind:checked={edits.pp3[section].Enabled as boolean} small></Checkbox>
	</summary>
	<div class="flex flex-col gap-2 px-4 py-2" class:opacity-70={!edits.pp3[section].Enabled}>
		{@render children()}
	</div>
</details>
